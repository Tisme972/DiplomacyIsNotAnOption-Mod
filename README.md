...
J'ai corrigé un bug de boxing de struct qui rendait inopérantes toutes les options de montant de ressources : `MinimumFood/Money/Wood/Stone/Iron/Souls` et les `Resource Gain Multipliers`. Le mod castait une struct vers l'interface `IUserUIResource` (ce qui crée une copie boxée), modifiait cette copie, puis réécrivait la struct d'origine qui elle n'avait pas bougé. Maintenant la valeur modifiée est bien recopiée (box → IncreaseAmount → unbox).

Pour rappel côté comportement : `MinimumMoney = 500` complète l'argent jusqu'à 500 uniquement quand tu es en dessous ; si t'as déjà plus, ça ne touche à rien, c'est voulu. Le contrôle est réappliqué à chaque tick, donc l'argent ne redescend plus durablement sous le seuil. Les autres fonctions (PV/dégâts/vitesse/portée unités et bâtiments, sources infinies, capacités) n'étaient pas concernées par ce bug.
